...
public class DAS extends DataSource {
	...
	public DAS(String serverPrefix) throws MalformedURLException, ParserConfigurationException, SAXException, IOException {
	...
	private List<Feature> getFeatures(String source, EntryPoint e) throws MalformedURLException, SAXException, IOException, ParserConfigurationException {
		...
		parser.parse(new URL(serverPrefix + "/das/" + source + "/features?segment=" + e.id + ":" + e.start + "," + e.stop).openStream(), featp);
		return featp.list;
	}
	...
	private StringBuffer getSequence(String ref, EntryPoint e) throws MalformedURLException, SAXException, IOException, ParserConfigurationException {
		SAXParser parser = SAXParserFactory.newInstance().newSAXParser();
		SequenceParser seqp = new SequenceParser();
		System.out.println(ref + "/dna?segment=" + e.id + ":" + e.start + "," + e.stop);
		parser.parse(new URL(ref + "/dna?segment=" + e.id + ":" + e.start + "," + e.stop).openStream(), seqp);
	...
	public List<EntryPoint> getEntryPoints(String ref) throws MalformedURLException, SAXException, IOException, ParserConfigurationException {
		if (entryPoints == null) {
			SAXParser parser = SAXParserFactory.newInstance().newSAXParser();
			entryPoints = new EntryPointParser();
			parser.parse(new URL(ref + "/entry_points").openStream(), entryPoints);
		}
