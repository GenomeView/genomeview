		...
		if (align != null) {
			...
			if (r.length() < 1000) {
				...
				for (int i = r.start(); i <= r.end(); i += grouping) {
					...
					conservation /= grouping;
					footprint /= grouping;
					conservationGP.lineTo((int) ((i - r.start()) * width*20), yOffset+(1-conservation)*3*lineHeigh);
					footprintGP.lineTo((int) ((i - r.start()) * width*20), yOffset+(1-footprint)*3*lineHeigh);
				...
				g.draw(footprintGP);
				g.drawString(this.displayName()+" ("+grouping+")",
						10,
						yOffset + 3*lineHeigh - 2);
				return 3*lineHeigh;
