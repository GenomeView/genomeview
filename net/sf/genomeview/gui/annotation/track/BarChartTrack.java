...
public class BarChartTrack extends Track {
	...
	public int paintTrack(Graphics2D g, Entry entry, int yOffset, double width) {
		...
		for (Location l : map.keySet()) {
			...
			int x1 = Convert.translateGenomeToScreen(l.start(), model.getAnnotationLocationVisible(), width);
			int x2 = Convert.translateGenomeToScreen(l.end() + 1, model.getAnnotationLocationVisible(), width);
			if (x2 == x1)
				x2++;
			if (x2 > 0) {
				...
				// );
				g.fillRect(x1, (int) (yOffset + (1 - heightScale) * line), x2 - x1, (int) (line * heightScale));
				...
				if (model.getLocationSelection().contains(l)) {
					...
					g.drawRect(x1, (int) (yOffset + (1 - heightScale) * line), x2 - x1, (int) (line * heightScale));
