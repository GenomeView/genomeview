...
public class MainWindow implements WindowListener, Observer {
    ...
    public MainWindow(String args[]) throws InterruptedException, ExecutionException {
        ...
        Option bogasO = parser.addHelp(parser.addStringOption("bogas"), "Start GenomeView with BOGAS data");

        Option hideChromViewO = parser.addHelp(parser.addBooleanOption("hideCV"), "Hides the chromosome panel.");
        Option hideStructureViewO = parser.addHelp(parser.addBooleanOption("hideSV"), "Hides the structure panel.");
        Option hideEvidenceViewO = parser.addHelp(parser.addBooleanOption("hideEV"), "Hides the evidence panel.");
        ...
        if (cmdFile == null && cmdUrl == null && cmdBogas == null) {
            ...
            try {
                data = new DataSource[] { new MobySource(new URI(cmdBogas).toURL()) };
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
        ...
        if (data != null) {
            ...
            for (DataSource ds : data) {
                final GVProgressBar pb = new GVProgressBar("Loading", "Loading data", window);
        ...
        Boolean hideChromView = (Boolean) parser.getOptionValue(hideChromViewO, Boolean.FALSE);
        Boolean hideStructureView = (Boolean) parser.getOptionValue(hideStructureViewO, Boolean.FALSE);
        Boolean hideEvidenceView = (Boolean) parser.getOptionValue(hideEvidenceViewO, Boolean.FALSE);
