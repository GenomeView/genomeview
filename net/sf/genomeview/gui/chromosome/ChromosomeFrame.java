...
public class ChromosomeFrame extends GridBagPanel implements Observer {
	...
	public class GenePaintLabel extends JLabel implements MouseListener,
			MouseMotionListener {
		...
		public void paintComponent(Graphics gg) {
			...
			if (totalHeight >= 0) {
				...
				featureView.drawAllChromosomes(g, windowWidth, windowHeight,
						totalHeight);
			} else {
				// TODO this is exactly the same as in fillShapeList, this can
				// be done more efficient!
				double precision=50;
				HashMap<Entry, FrequencyMap> density = new HashMap<Entry, FrequencyMap>();
				int start = model.getChromosomeLocationVisible().start();
				int end = model.getChromosomeLocationVisible().end();
				int length = (int)((end - start + 1) / precision);
				for (Entry chrom : model.entries()) {
					density.put(chrom, new FrequencyMap());
					...
					if (visibleSource) {
						for (Type type : Type.values()) {
							if (model.isVisibleOnChromosome(type)) {
								List<Feature> features = chrom.annotation
										.getByType(type, model
												.getChromosomeLocationVisible());
								for (Feature f : features) {
									for (int i = 0; i < precision; i++) {
										if (f.overlaps(new Location(i * length
												+ start, (i + 1) * length
												+ start)))
											density.get(chrom).count(i);
									}
								}
							}
						}
				...
				this.setPreferredSize(new Dimension(windowWidth, 40*model.entries().size()));
				this.revalidate();
				int yOffset=0;
				for (Entry e : model.entries()) {
					double max=0;
					for(int i:density.get(e).values())
						if(i>max)
							max=i;
//					System.out.println("Total: "+total);
		...
		public void doubleClickFeatureView(MouseEvent e) {
			if (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2) {
				...
				if (hits.isFeatureHit()) {
					...
					int l = feat.length();
					model.setAnnotationLocationVisible(new Location(feat
							.start()
							- (l / 20), feat.end() + (l / 20)));
				} else if (hits.isTrackHit()) {
					Location visibleChrom = model
							.getChromosomeLocationVisible();
		...
		private void getFeatureHits(Hits hits, DrawableChromosome entryHit,
				int x, int y) {
		...
		private DrawableChromosome getTrackHits(Hits hits, int y) {
			...
			for (DrawableChromosome dcEntry : featureView.getAllShapes()) {
				int trackThickness = dcEntry.thickness
						* ChromosomeView.SHAPEHEIGHT + ChromosomeView.MARGIN
						* 2;
		...
		public void mouseMoved(MouseEvent e) {
			...
			if (hits.isFeatureHit()) {
				...
				floatingWindow.setLocation(e.getXOnScreen() + 5, e
						.getYOnScreen() + 5);
	...
	public ChromosomeFrame(Model model) {
		...
		gc.gridwidth = 1;
		buttonContainer
				.add(new JButton(new ChromosomeZoomInAction(model)), cgc);
		cgc.gridx++;
		buttonContainer.add(new JButton(new ChromosomeZoomOutAction(model)),
				cgc);
		cgc.gridy++;
		cgc.gridx--;
		cgc.gridwidth = 2;
		buttonContainer.add(new JButton(new ChromosomeOverviewAction(model)),
				cgc);
		cgc.gridy++;
		cgc.gridwidth = 1;
		buttonContainer.add(new JButton(new ChromosomeMoveLeftAction(model)),
				cgc);
		cgc.gridx++;
		buttonContainer.add(new JButton(new ChromosomeMoveRightAction(model)),
				cgc);
		cgc.gridx--;
	...
	public class Slider extends JPanel implements Observer, ChangeListener {
		...
		public void stateChanged(ChangeEvent e) {
			...
			model.setChromosomeLocationVisible(new Location(brm.getValue(), brm
					.getValue()
					+ brm.getExtent()));
			model.addObserver(this);
		...
		public void update(Observable o, Object arg) {
			...
			brm.setRangeProperties(r.start(), r.end() - r.start(), 0, model
					.getLongestChromosomeLength(), false);
		}
