...
class PileupTask extends Task {
	...
	public PileupTask(Data<Pile> pw, int idx, PileupSummary summary,
			Model model) {
		super(new Location(idx * PileupSummary.CHUNK,
	...
	public void run() {
		try {
			...
			Iterable<Pile> piles = pw.get(idx * PileupSummary.CHUNK,
					(idx + 1) * PileupSummary.CHUNK);
			for (Pile p : piles) {
				if (p.start() >= idx * PileupSummary.CHUNK
						&& p.start() < (idx + 1) * PileupSummary.CHUNK) {
			...
			model.getLog().log(Level.WARNING,
					"Scheduler problem: " + pw + "\t" + idx + "\tpw.get("
							+ idx * PileupSummary.CHUNK + ", "
							+ (idx + 1) * PileupSummary.CHUNK + ")\n\t" + e);
		}
	}
}
