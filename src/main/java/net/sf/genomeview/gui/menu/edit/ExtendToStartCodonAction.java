...
public class ExtendToStartCodonAction extends AbstractModelAction {
	...
	public ExtendToStartCodonAction(Model model) {
		super(MessageManager.getString("editmenu.extend_to_next_start_codon"),
				model);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (rf.strand() == Strand.FORWARD) {
			...
			while (!hasReverseStart(start)
					&& start < model.vlm.getVisibleEntry().getMaximumLength()) {
				start++;
			}
	...
	private boolean hasReverseStart(int start) {
		String codon = SequenceTools.reverseComplement(model.vlm
