	...
	public void actionPerformedSafe(ActionEvent arg0) {
		...
		if (input != null && input.trim().length() > 0) {
			...
			for (Track t : model.getTrackList()) {
				if (t.getDataKey().toString().toLowerCase().contains(input) || t
						.config().displayName().toLowerCase().contains(input)) {
					hits.add(t);
				}

			}
			if (hits.size() > 0) {
				model.getGUIManager().getEvidenceLabel()
						.scroll2track(hits.get(0));
			}
