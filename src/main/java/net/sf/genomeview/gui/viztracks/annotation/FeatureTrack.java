...
public class FeatureTrack extends Track {
			...
			if (x2 > 0) {
				...
				for (Location l : loc) {
					...
					int subX2 = Convert.translateGenomeToScreen(l.end() + 1,
							model.vlm.getAnnotationLocationVisible(), width);
					Rectangle rec = new Rectangle(subX1,
							thisLine * lineThickness, subX2 - subX1,
							lineThickness - 5);
				...
				case REVERSE:// reverse arrow
					...
					g.drawLine(x1 - trianglehalf,
							thisLine * lineThickness + trianglehalf, x1,
							thisLine * lineThickness + lineThickness - 5);
					break;
				case FORWARD:// forward arrow
					g.drawLine(x2, thisLine * lineThickness, x2 + trianglehalf,
							thisLine * lineThickness + trianglehalf);
					g.drawLine(x2 + trianglehalf,
							thisLine * lineThickness + trianglehalf, x2,
							thisLine * lineThickness + lineThickness - 5);
					break;
				default:// do nothing
					break;
				...
				if (forceLabels || x2 - x1 > 100) {
					int a = Convert.translateGenomeToScreen(loc[0].start(),
							model.vlm.getAnnotationLocationVisible(), width);
	...
	private boolean doubleClick(MouseEvent e) {
		...
		if (locationHit != null) {
			...
			model.vlm.setAnnotationLocationVisible(new Location(
					featHit.start() - (l / 20), featHit.end() + (l / 20)));
			return true;
		}
		return false;
	}
	...
	private void drawRects(Graphics g, ArrayList<Rectangle> rectList,
		...
		for (Rectangle rect : rectList) {
			...
			lastPoint = new Point(rect.x + rect.width,
		...
		if (!e.isConsumed()) {
			...
			if (Mouse.button1(e)) {
				if (locationHit == null && !Mouse.modifier(e)) {
					...
					if (model.selectionModel().getLocationSelection()
							.contains(locationHit)) {
						model.selectionModel()
								.removeLocationSelection(locationHit);
					} else {
						model.selectionModel()
		...
		protected GridBagPanel getGUIContainer() {
			...
			final JButton filter = new JButton(new AbstractAction(
				...
				public void actionPerformed(ActionEvent e) {
					...
					final DoubleJSlider slider = new DoubleJSlider(minScore,
							maxScore, getThreshold() > minScore ? getThreshold()
									: minScore,
						...
						public void stateChanged(ChangeEvent changeEvent) {
							...
							if (!theSlider.getValueIsAdjusting()) {
								optionPane.setInputValue(
										theSlider.getDoubleValue());
								theSlider.setDoubleValue(
										theSlider.getDoubleValue());
							}
						}
					};
