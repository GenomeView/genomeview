...
public class SyntenicTrack extends Track {
	 ...
	 * @param ref    the {@link DataKey} referring to -presumably - the key
	 *               under which the syntenic data is stored.
...
//		}
		...
		final List<SyntenicBlock> list = model.entries().syntenic.get(entry,
				null);
		final Location visible = model.vlm.getAnnotationLocationVisible();

		double colorBlockLength = (entry.sequence().size() + 1.0) / colors;
		/* Reference color scheme in 20 steps */
		// dataKey should hold reference track name
		if (target.equals(dataKey.toString())) {
			for (int i = 0; i <= 20; i++) {
				int length = visible.end() - visible.start() + 1;
				int start = visible.start() + (int) (length / 20.0 * i);
				...
				int screenStart = Convert.translateGenomeToScreen(start,
						visible, width);
				int screenEnd = Convert.translateGenomeToScreen(end, visible,
						width);
