...
//		add(new SearchButtons(), gc);
	 ...
	 * 
	 * @param gc    {@link GridBagConstraints} to tweak the way the new panel
	 *              should be added.
	 * @param panel
	 */
	...
	class MonitorPanel extends GridBagPanel {
		...
		public MonitorPanel() {
			...
			gc.gridx++;
			gc.weightx = 0;
			this.add(model.getConnectionMonitor().networkLabel, gc);
			gc.gridx++;
			this.add(model.getConnectionMonitor().reposLabel, gc);
			gc.gridx++;
			this.add(model.getConnectionMonitor().webLabel, gc);
		}
	}
